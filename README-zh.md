...
| 组件 | 版本 |
|---|---|
| DeepSeek Harness | **`dsh-v0.1.5-rc.1`**（GitHub tag；2026-09-10 已核验：类型门 + 单元/装配测试 + 制品构建）。已发布 npm 线 `0.1.5-rc.1`（npm `latest`/`next` 已是 `0.1.5-rc.1`；peer 依赖 `>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0`）。 |
...
- `typecheck` 经本地 harness checkout 解析 `@deepseek-ai/*`；`typecheck:ci` 对照已发布的 `0.1.5-rc.1` 类型。
