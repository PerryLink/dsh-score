...
| 组件 | 版本 |
|---|---|
| DeepSeek Harness | **`dsh-v0.1.7-alpha.2`**（GitHub tag；peer 区间已承认 alpha.2 线：`>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0 \|\| >=0.1.7-0 <0.2.0`）。dev/test 钉号与 `typecheck:ci` 尺子现对照已发布的 `0.1.7-alpha.2` 线（`0.1.6-alpha.2` 面已于 2026-09-18 核验：类型门 + 单元/装配测试 + 制品构建）；`ctx.jobs` 接缝已迁移到 alpha.2 的 `SessionId` 契约。 |
...
- `typecheck` 经本地 harness checkout 解析 `@deepseek-ai/*`；`typecheck:ci` 对照已发布的 `0.1.7-alpha.2` 类型。
