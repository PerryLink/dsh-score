...
| Componente | Versión |
|---|---|
| DeepSeek Harness | **`dsh-v0.1.7-alpha.2`** (etiqueta de GitHub; el rango de pares admite la línea alpha.2: `>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0 \|\| >=0.1.7-0 <0.2.0`). Los pines de dev/test y la regla `typecheck:ci` ahora miden la línea publicada `0.1.7-alpha.2` (la cara `0.1.6-alpha.2` se verificó el 2026-09-18: puertas de tipos, suites unitarias/de ensamblaje, build de artefactos); el seam `ctx.jobs` se migró al contrato `SessionId` de alpha.2. |
