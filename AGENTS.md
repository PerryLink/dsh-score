...
- `tests/` — vitest; real Cordis `Context` + real `SessionStore`/`Session`/`ToolRuntime`/`AgentRegistry`/`LocalJobRegistry`/`Storage`+`DomainFacility` from the pinned `0.1.7-rc.1` peers; the subprocess provider is a scripted subclass of the REAL `SubprocessRuntime`.
